...
function App() {
  ...
  const stars = useMemo(() => {
    const starArray = []
    for (let i = 0; i < 150; i++) { // Reduced from 200 to 150 for better performance
      starArray.push({
        id: i,
  ...
  const getButtonStyle = (buttonId) => {
    ...
    return {
      background: isHovered ? 'rgba(255, 255, 255, 0.35)' : 'rgba(255, 255, 255, 0.25)',
      ...
      boxShadow: isHovered ? '0 8px 25px rgba(0, 0, 0, 0.4)' : '0 4px 12px rgba(0, 0, 0, 0.3)'
    }
  }
      ...
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          ...
          try {
            ...
            const placeInfo = await reverseGeocode(position.coords.latitude, position.coords.longitude)
            ...
            analyzeStargazingConditions(position.coords.latitude, position.coords.longitude)
          } catch (error) {
    ...
    try {
      ...
      if (!isNight) {
        ...
        setStargazingPrediction({
          ...
          recommendation: `It's ${timeString}. Stars appear after sunset, around 8 PM.`,
          weatherData: {
            cloudCover: 0,
            visibility: 0,
            humidity: 0,
          ...
          timeInfo: {
            ...
            isNightTime: false,
            nextSunset: '8:00 PM' // This would be calculated from sunrise/sunset API
          }
        })
        return
      }
      ...
      try {
        ...
        const isGoodForStargazing = isGoodCloudCover && isGoodVisibility && isGoodHumidity && isGoodLightPollution

        let recommendation = ""
        if (isGoodForStargazing) {
        ...
        setStargazingPrediction({
          ...
          recommendation: lightPollution.factor > 0.5
  ...
  const handleLocationSubmit = async (e) => {
    ...
    try {
      ...
      if (coordinates) {
        ...
        analyzeStargazingConditions(coordinates.latitude, coordinates.longitude)
        setShowLocationForm(false)
      } else {
        alert('Location not found. Please check the city and country names and try again.')
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <header className="relative z-10 px-6 pt-2 flex justify-between items-start">
        <div>
          <h1 className="text-4xl font-bold text-white" style={{
            textShadow: '0 0 5px #fff, 0 0 10px #fff, 0 0 15px #fff, 0 0 20px #fff, 0 0 25px #fff, 0 0 30px rgba(102, 126, 234, 0.6)',
            letterSpacing: '0.05em'
          }}>Star Gazing Tonight</h1>
          <p className="text-gray-300">Will you be able to see the stars?</p>
        </div>
            ...
            <div className="mb-2">
              ...
              {!userLocation ? (
                ...
                <div className="space-y-4">
                  <div className={`text-center py-2 px-4 rounded-lg ${stargazingPrediction.isGoodForStargazing
                    ? 'bg-green-900/30 border border-green-500/30'
                    : 'bg-red-900/30 border border-red-500/30'
                    }`}>
                    <h3 className="text-lg font-semibold text-white" style={{ margin: '0', paddingTop: '8px', paddingBottom: '8px' }}>
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    ...
                    >
                      <div className="text-sm text-gray-400" style={{ margin: '0' }}>Cloud Cover</div>
                      <div className="text-white font-semibold" style={{ margin: '0' }}>
                        {stargazingPrediction.weatherData.cloudCover > 0 ? stargazingPrediction.weatherData.cloudCover.toFixed(1) + '%' : 'N/A'}
                      </div>
                      <div className={`text-xs ${stargazingPrediction.factors.cloudCover === 'Good' ? 'text-green-400' : stargazingPrediction.factors.cloudCover === 'Poor' ? 'text-red-400' : 'text-gray-400'}`} style={{ margin: '0' }}>
                    ...
                    >
                      <div className="text-sm text-gray-400" style={{ margin: '0' }}>Visibility</div>
                      <div className="text-white font-semibold" style={{ margin: '0' }}>
                        {stargazingPrediction.weatherData.visibility > 0 ? stargazingPrediction.weatherData.visibility.toFixed(1) + ' km' : 'N/A'}
                      </div>
                      <div className={`text-xs ${stargazingPrediction.factors.visibility === 'Good' ? 'text-green-400' : stargazingPrediction.factors.visibility === 'Poor' ? 'text-red-400' : 'text-gray-400'}`} style={{ margin: '0' }}>
                    ...
                    >
                      <div className="text-sm text-gray-400" style={{ margin: '0' }}>Humidity</div>
                      <div className="text-white font-semibold" style={{ margin: '0' }}>
                        {stargazingPrediction.weatherData.humidity > 0 ? stargazingPrediction.weatherData.humidity.toFixed(1) + '%' : 'N/A'}
                      </div>
                      <div className={`text-xs ${stargazingPrediction.factors.humidity === 'Good' ? 'text-green-400' : stargazingPrediction.factors.humidity === 'Poor' ? 'text-red-400' : 'text-gray-400'}`} style={{ margin: '0' }}>
                    ...
                    >
                      <div className="text-sm text-gray-400" style={{ margin: '0' }}>Light Pollution</div>
                      <div className="text-white font-semibold" style={{ margin: '0' }}>
                        {stargazingPrediction.lightPollution ? stargazingPrediction.lightPollution.level : 'N/A'}
                      </div>
                      <div className={`text-xs ${stargazingPrediction.factors.lightPollution === 'Good' ? 'text-green-400' : stargazingPrediction.factors.lightPollution === 'Poor' ? 'text-red-400' : 'text-gray-400'}`} style={{ margin: '0' }}>
            ...
            <div className="mb-2">
              ...
              >
                <p className="text-sm text-white" style={{ margin: '0' }}>
                  Note: Your location is private. All calculations happen directly on your device.
                  Your location is never sent to any server.
                </p>
              </div>
            ...
            {userLocation && (
              ...
              >
                <div className="text-green-300 text-sm">
                  <div className="text-green-200 font-semibold mb-1">Your Location</div>
      ...
      {showLocationForm && (
        <div className="fixed inset-0 modal-backdrop flex items-center justify-center z-50">
          <div className="prediction-card rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-semibold text-white mb-4">Enter Your Location</h3>
      ...
      {showManualForm && (
        <div className="fixed inset-0 modal-backdrop flex items-center justify-center z-50">
          <div className="prediction-card rounded-lg p-6 max-w-md w-full mx-4">
            <h3 className="text-xl font-semibold text-white mb-4">Enter Coordinates</h3>
